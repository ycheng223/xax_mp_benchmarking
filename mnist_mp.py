"""Trains a simple convolutional neural network on the MNIST dataset with mixed precision.

Run this example with `python -m examples.mnist_mp`.
"""
...
@dataclass
class Config(xax.MixedPrecisionConfig, xax.SupervisedConfig):
    ...
    # Added for testing:
    valid_every_n_steps: int = xax.field(100, help="Run validation every N steps")
    valid_every_n_seconds: float = xax.field(100.0, help="Run validation every N seconds")
    max_steps: int = xax.field(1000, help="Maximum number of training steps")
...
class MnistClassification(xax.MixedPrecisionMixin[Config], xax.SupervisedTask[Config]):
    ...
    def get_data_iterator(self, phase: xax.Phase, key: PRNGKeyArray) -> Iterator:
        print(f"DEBUG: Loading data for phase: {phase}")
        ds = MNIST(train=phase == "train", root_dir=xax.get_data_dir() / "mnist", dtype="float32")
        print(f"DEBUG: Dataset loaded, train={phase == 'train'}, size={len(ds.images)}")
